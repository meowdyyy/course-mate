...
//             url: filePath, //Using 'url' for consistency with frontend
//             filePath: filePath,
...
if (!isServerless) {
    const uploadDirs = [ 'uploads/course-materials', 'uploads/documents', 'uploads/videos', 'uploads/profiles' ];
    uploadDirs.forEach(dir => {
        try { if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true }); } catch(e){ console.error('Failed to create directory', dir, e); }
    });
} else {
    console.log('Serverless environment: skipping creation of local upload directories.');
}

//Storage configuration for course materials
const materialStorage = isServerless
    ? multer.memoryStorage()
    : multer.diskStorage({
            destination: function (req, file, cb) {
                const type = req.body.type || 'course-material';
                let uploadPath = 'uploads/course-materials';
                if (type === 'video') uploadPath = 'uploads/videos';
                else if (['document','pdf','note'].includes(type)) uploadPath = 'uploads/documents';
                cb(null, uploadPath);
            },
            filename: function (req, file, cb) {
                const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
...
//Upload middleware for course materials
const uploadCourseMaterial = isServerless
    ? (req,res,next)=> res.status(503).json({ message: 'File uploads disabled in serverless deployment. Use external storage (e.g. S3, Cloudinary).' })
    : multer({
            storage: materialStorage,

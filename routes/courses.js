...
router.get('/resources', auth, async (req, res) => {
  ...
  // Semester applies to resource-level semester, not course document; handled after unwind
    if (q) {
      courseMatch.$or = [
    ...
    // Aggregate resources from courses
    const pipeline = [
  { $match: courseMatch },
  ...
  ...(semester ? [{ $match: { 'studentResources.semester': { $regex: semester, $options: 'i' } } }] : []),
    ...
    res.json({ resources: data, semesters, pagination: { page: pg, limit: lim, total, pages: Math.ceil(total/lim) } });
  } catch (e) {
...
router.post('/', [
  ...
  owner: req.user._id,
      ...
      isApproved: true // auto-approved since only admin can create
    };
...
// Removed former instructor-specific listing route
...
router.post('/:id/student-resources', [
  ...
  const { title, type, url, filename, description, semester } = req.body;
    ...
    // Semester required (e.g., 'Summer 2025') simple pattern: Word + space + 4 digits
    if (!semester || !/^[A-Za-z]+\s+\d{4}$/.test(semester)) {
      return res.status(400).json({ message: 'Valid semester (e.g., Summer 2025) is required' });
    }
...
router.get('/:id/student-resources', auth, async (req, res) => {
  try {
    ...
    const responseResources = paged.map(r => {
      ...
      // Don't expose full ratings/comments arrays in list view to reduce payload size
      delete obj.ratings;
      delete obj.comments;
...
router.patch('/:id/student-resources/:resourceId', auth, async (req, res) => {
  ...
  if (!isOwner && !isAdmin) {
    ...
    if (semester) {
      if (!/^[A-Za-z]+\s+\d{4}$/.test(semester)) return res.status(400).json({ message: 'Semester must be like Summer 2025' });
...
router.put('/:id/material/:materialId', [
  ...
  try {
    ...
    // Only admin can update materials (instructor role removed)
    if (req.user.role !== 'admin') {
...
// @access  Private (Admin only)
// Removed /:id/performance endpoint (assignments/grades feature deprecated)
